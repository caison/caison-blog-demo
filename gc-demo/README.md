...
设置忽略System.gc()的调用，不建议设置该参数，因为部分依赖Java NIO的框架(例如Netty)在内存异常耗尽时，会主动调用System.gc()，触发Full GC，回收DirectByteBuffer对象，以此作为最后保障机制，从而回收堆外内存，设置该参数之后会导致在该情况下堆外内存得不到清理
